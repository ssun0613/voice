...
from ssun.FastPitchForman.utils.tools import get_mask_from_lengths, pad
from ssun.FastPitchForman.text.symbols import symbols
...
class Decoder(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        self.fc_layers = nn.ModuleList(
            [
                LinearNorm(d_model, n_mel_channels) for _ in range(n_layers)
            ]
        )
...
class Generator(nn.Module):
    ...
    def __init__(self, config, query_projection=False):
        ...
        self.layer_stack = nn.ModuleList(
            [
                FFTBlock( d_model, n_head, d_k, d_v, d_inner, kernel_size, dropout=dropout, query_projection=query_projection) for _ in range(n_layers)
            ]
        )
