...
from model import FastPitchFormantLoss
...
def main(args, configs):
    ...
    Loss = FastPitchFormantLoss(preprocess_config, model_config).to(device)
    print("Number of FastPitchFormant Parameters:", num_param)
    ...
    while True:
        ...
        for batchs in loader:
            for batch in batchs:
                ...
                # (ids, raw_texts, speakers, texts, text_lens, max(text_lens), mels, mel_lens, max(mel_lens), pitches, durations)
                # Forward
                ...
                if step % log_step == 0:
                    ...
                    message2 = "Total Loss: {:.4f}, Mel Loss: {:.4f}, Pitch Loss: {:.4f}, Duration Loss: {:.4f}".format(*losses)
                ...
                if step % save_step == 0:
                    torch.save({"model": model.module.state_dict(), "optimizer": optimizer._optimizer.state_dict(),}, os.path.join(train_config["path"]["ckpt_path"],  "{}.pth.tar".format(step),))
