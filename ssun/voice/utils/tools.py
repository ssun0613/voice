...
def to_device(data, device):
    if len(data) == 11:
        (ids, raw_texts, speakers, texts, src_lens, max_src_len, mels, mel_lens, max_mel_len, pitches, durations) = data
        ...
        return (ids, raw_texts, speakers, texts, src_lens, max_src_len, mels, mel_lens, max_mel_len, pitches, durations)
...
def get_mask_from_lengths(lengths, max_len=None):
    ...
    ids = torch.arange(0, 192).unsqueeze(0).expand(batch_size, -1).to(device)
    mask = ids >= lengths.unsqueeze(1).expand(-1, 192)
...
def synth_one_sample(targets, predictions, vocoder, model_config, preprocess_config):
    ...
    fig = plot_mel(
        [(mel_prediction.cpu().numpy(), pitch), (mel_target.cpu().numpy(), pitch)],stats,
    ...
    if vocoder is not None:
        ...
        wav_reconstruction = vocoder_infer(mel_target.unsqueeze(0), vocoder, model_config, preprocess_config)[0]
        wav_prediction = vocoder_infer(mel_prediction.unsqueeze(0), vocoder, model_config, preprocess_config)[0]
    else:
