...
class Conv1DBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, activation=None, dropout=None):
        ...
        self.conv_layer.add_module("conv_layer",
            ConvNorm(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=int((kernel_size - 1) / 2), dilation=1, w_init_gain="tanh"))
...
class FFTBlock(nn.Module):
    ...
    def forward(self, enc_input, mask=None, slf_attn_mask=None, hidden_query=None):
        enc_output, enc_slf_attn = self.slf_attn(self.query_linear(enc_input + hidden_query) if hidden_query is not None else enc_input, enc_input, enc_input, mask=slf_attn_mask)
        if mask is not None:
