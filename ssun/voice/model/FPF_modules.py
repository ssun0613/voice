...
class TextEncoder(nn.Module):
    ...
    def forward(self, src_seq, mask, return_attns=False):
        ...
        enc_slf_attn_list = []
        batch_size, max_len = src_seq.shape[0], src_seq.shape[2]
...
class VarianceAdaptor(nn.Module):
    ...
    def forward(self, x, speaker_embedding, src_mask, mel_mask=None, max_len=None, pitch_target=None, duration_target=None, p_control=1.0, d_control=1.0):

        # log_duration_prediction.shape : torch.Size([2, 16])
        # pitch_prediction.shape : torch.Size([2, 16])
        # pitch_embedding.shape : torch.Size([2, 16, 256])
        ...
        x, duration_rounded, mel_len, mel_mask = self.upsample(torch.cat([x, pitch_embedding], dim=-1), mel_mask, max_len, log_duration_prediction=log_duration_prediction, duration_target=duration_target, d_control=d_control)

        text_hidden, pitch_hidden = torch.split(x, x.shape[-1]//2, dim=-1)

        return (text_hidden, pitch_hidden, pitch_prediction, log_duration_prediction, duration_rounded, mel_len, mel_mask)
    ...
    def __init__(self, model_config):
        ...
        self.conv_layer = nn.Sequential(OrderedDict(
                [("conv1d_1",
                  ...
                  ("conv1d_2",
                  Conv(self.filter_size, self.filter_size, kernel_size=self.kernel, padding=1)),
                  ("relu_2", nn.ReLU()),
...
class Generator(nn.Module):
    ...
    def __init__(self, config, query_projection=False):
        ...
        self.layer_stack = nn.ModuleList([FFTBlock(d_model, n_head, d_k, d_v, d_inner, kernel_size, dropout=dropout, query_projection=query_projection) for _ in range(n_layers)])
    ...
    def forward(self, hidden, mask, hidden_query=None):
        ...
        if not self.training and hidden.shape[1] > self.max_seq_len:
            output = hidden + get_sinusoid_encoding_table( hidden.shape[1], self.d_model)[: hidden.shape[1], :].unsqueeze(0).expand(batch_size, -1, -1).to(hidden.device)
        else:
            output = hidden + self.position_enc[:, :max_len, :].expand(batch_size, -1, -1)

        for i, enc_layer in enumerate(self.layer_stack):
            output, enc_slf_attn = enc_layer(output, mask=mask, slf_attn_mask=slf_attn_mask, hidden_query=hidden_query if i==0 else None)
...
class Decoder(nn.Module):
    ...
    def __init__(self, preprocess_config, model_config):
        ...
        d_k = d_v = (model_config["transformer"]["decoder_hidden"] // model_config["transformer"]["decoder_head"])
...
class LengthRegulator(nn.Module):
    ...
    def LR(self, x, duration, max_len):
        ...
        # return output, torch.LongTensor(max_len).to(device)
