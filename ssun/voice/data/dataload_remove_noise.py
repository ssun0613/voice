...
class accent():
    ...
    def __getitem__(self, index):
        ...
        sp_id, sp_id_1 = self.data_sp_id(self.dataset_mel[index % self.dataset_size])
        return mel_tmp, pitch_tmp, sp_id, sp_id_1
    ...
    def data_sp_id(self, label_path):
        ...
        sp_id_1 = LABEL[data_sp_id_lower]
        return sp_id, sp_id_1
    ...
    def data_load_npy(self):
        ...
        mel_data.append(dataset_mel[6784])
        mel_data.append(dataset_mel[12345])
        mel_data.append(dataset_mel[17890])
        mel_data.append(dataset_mel[21098])
        mel_data.append(dataset_mel[-1])

        pitch_data.append(dataset_pitch[0])
        pitch_data.append(dataset_pitch[6784])
        pitch_data.append(dataset_pitch[12345])
        pitch_data.append(dataset_pitch[17890])
        pitch_data.append(dataset_pitch[21098])
...
class MyCollator(object):
    ...
    def __call__(self, batch):
        ...
        for token in batch:
            ...
            new_batch.append((mel_pad, pitch_pad, len_crop[0], sp_id, sp_id_1, mel_pad.shape[0]))
        ...
        mel_len = torch.from_numpy(np.stack(f, axis=0))
        ...
        sp_id_1 = torch.from_numpy(np.stack(e, axis=0))

        return {'melsp': melsp, 'mel_len': mel_len, 'pitch': pitch, 'len_org': len_org, 'sp_id': sp_id, 'sp_id_1': sp_id_1}
