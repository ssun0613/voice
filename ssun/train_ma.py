...
class MaskCycleGANVCTraining(object):
    ...
    def __init__(self, args):
        """Args:args (Namespace): Program arguments from argparser"""
        # Store args
        ...
        self.dataset_A = self.loadPickleFile(os.path.join(args.preprocessed_data_dir, args.speaker_A_id, f"{args.speaker_A_id}_normalized.pickle"))
        dataset_A_norm_stats = np.load(os.path.join(args.preprocessed_data_dir, args.speaker_A_id, f"{args.speaker_A_id}_norm_stat.npz"))
        self.dataset_A_mean = dataset_A_norm_stats['mean']
        self.dataset_A_std = dataset_A_norm_stats['std']

        # Initialize speakerB's dataset
        self.dataset_B = self.loadPickleFile(os.path.join(args.preprocessed_data_dir, args.speaker_B_id, f"{args.speaker_B_id}_normalized.pickle"))
        dataset_B_norm_stats = np.load(os.path.join(args.preprocessed_data_dir, args.speaker_B_id, f"{args.speaker_B_id}_norm_stat.npz"))
        ...
        d_params = list(self.discriminator_A.parameters()) + list(self.discriminator_B.parameters()) + \
                   list(self.discriminator_A2.parameters()) + list(self.discriminator_B2.parameters())
    ...
    def adjust_lr_rate(self, optimizer, generator):
        """Decays learning rate.
            Args : optimizer (torch.optim): torch optimizer
    ...
    def loadPickleFile(self, fileName):
        """Loads a Pickle file.
        Args:fileName (str): pickle file path
        Returns:file object: The loaded pickle file object
        """
        ...
        for epoch in range(self.start_epoch, self.num_epochs + 1):
            ...
            for i, (real_A, mask_A, real_B, mask_B) in enumerate(tqdm(self.train_dataloader)):
                ...
                with torch.set_grad_enabled(True):
                    ...
                    g_loss = g_loss_A2B + g_loss_B2A + generator_loss_A2B_2nd + generator_loss_B2A_2nd + \
