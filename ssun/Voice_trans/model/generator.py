...
class generator(nn.Module):
    def __init__(self, opt):
        ...
        if not opt.debugging:
            ...
            from Voice_trans.model.pitch_predictor import pitch_predictor_original as P
...
class generator_without_content(nn.Module):
    def __init__(self, opt):
        super(generator_without_content,self).__init__()
        if not opt.debugging:
            from ssun.Voice_trans.model.encoder import Er, Ec, InterpLnr
            from ssun.Voice_trans.model.decoder_s import Decoder_s as Ds
            from ssun.Voice_trans.model.pitch_predictor import pitch_predictor as P
            ...
            from Voice_trans.model.encoder import Er, Ec, InterpLnr
            from Voice_trans.model.decoder_s import Decoder_s as Ds
            from Voice_trans.model.pitch_predictor import pitch_predictor_without_content as P
            from Voice_trans.functions.etc_fcn import quantize_f0_torch
    ...
    def forward(self, mel_in, len_org, sp_id):
        ...
        # r_s = torch.cat((rhythm_repeat, sp_id.unsqueeze(1).expand(-1, mel_in.transpose(2,1).size(-1), -1)), dim=-1)
        ...
        # r_p_r = torch.cat((rhythm_r_repeat, sp_id.unsqueeze(1).expand(-1, mel_in.transpose(2,1).size(-1), -1)), dim=-1)
...
class generator_only_speakerid(nn.Module):
    def __init__(self, opt):
        super(generator_only_speakerid,self).__init__()
        if not opt.debugging:
            from ssun.Voice_trans.model.encoder import Er, Ec, InterpLnr
            from ssun.Voice_trans.model.decoder_s import Decoder_s as Ds
            from ssun.Voice_trans.model.pitch_predictor import pitch_predictor_only_speakerid as P
            from ssun.Voice_trans.functions.etc_fcn import quantize_f0_torch
        else:
            from Voice_trans.model.encoder import Er, Ec, InterpLnr
            from Voice_trans.model.decoder_s import Decoder_s as Ds
            from Voice_trans.model.pitch_predictor import pitch_predictor_only_speakerid as P
            from Voice_trans.functions.etc_fcn import quantize_f0_torch
    ...
    def forward(self, mel_in, len_org, sp_id):
        ...
        sp_id = sp_id.unsqueeze(1).expand(-1, mel_in.transpose(2,1).size(-1), -1)
        pitch_p = self.P(sp_id).repeat_interleave(8, dim=1)
        pitch_p_repeat_quan = self.quantize_f0_torch(pitch_p)[0]
        ...
        pitch_p_r = self.P(sp_id).repeat_interleave(8, dim=1)
        pitch_p_r_quan = self.quantize_f0_torch(pitch_p_r)[0]
