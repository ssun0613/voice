...
class generator_original(nn.Module):
    def __init__(self, opt):
        ...
        if not opt.debugging:
            ...
            from ssun.Voice_trans.model.pitch_predictor import pitch_predictor_original as P
    ...
    def forward(self, mel_in, len_org, sp_id):
        ...
        # r_c_s = torch.cat((rhythm_repeat, content_repeat), dim=-1)
        ...
        # r_c_p_r = torch.cat((rhythm_r_repeat, content_r_repeat),dim=-1)
...
class generator_without_rhythm(nn.Module):
    def __init__(self, opt):
        ...
        if not opt.debugging:
            from ssun.Voice_trans.model.encoder import Er, Ec, InterpLnr
            from ssun.Voice_trans.model.decoder_s import Decoder_s as Ds
            from ssun.Voice_trans.model.pitch_predictor import pitch_predictor_without_content as P
            from ssun.Voice_trans.functions.etc_fcn import quantize_f0_torch
        else:
            from Voice_trans.model.encoder import Er, Ec, InterpLnr
            from Voice_trans.model.decoder_s import Decoder_s as Ds
    ...
    def forward(self, mel_in, len_org, sp_id):
        ...
        pitch_p = self.P(content_repeat).repeat_interleave(8, dim=1)
        pitch_p_repeat_quan = self.quantize_f0_torch(pitch_p)[0]
        ...
        pitch_p_r = self.P(content_r_repeat).repeat_interleave(8, dim=1)
        pitch_p_r_quan = self.quantize_f0_torch(pitch_p_r)[0]
