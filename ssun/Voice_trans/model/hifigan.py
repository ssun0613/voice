...
class ResBlock1(nn.Module):
    def __init__(self, h, channels, kernel_size=3, dilation=(1, 3, 5)):
        ...
        init_weights_(self.convs1)
        init_weights_(self.convs2)

    def forward(self, x):
        for c1, c2 in zip(self.convs1, self.convs2):
            xt = F.leaky_relu(x, LRELU_SLOP)
            xt = c1(xt)
            xt = F.leaky_relu(xt, LRELU_SLOP)
            xt = c2(xt)
            x = xt + x
        return x
...
class Generator(nn.Module):
    def __init__(self, h):
        ...
        self.h = h
        self.n_mel = h.num_mels
        self.num_kernels = len(h.resblock_kernel_sizes)
        self.num_upsamples = len(h.upsample_rates)
        self.conv_pre = weight_norm(Conv1d(self.n_mel, h.upsample_initial_channel, 7, 1, padding=3))
        resblock = ResBlock1
        ...
        init_weights_([self.conv_pre])
        init_weights_(self.ups)
        init_weights_([self.conv_post])

    def forward(self, x):
        x = self.conv_pre(x)
        for i in range(self.num_upsamples):
            x = F.leaky_relu(x, LRELU_SLOP)
            x = self.ups[i](x)
            xs = None
...
class DiscriminatorP(nn.Module):
    def __init__(self, period, kernel_size=5, stride=3, use_spectral_norm=False):
        ...
        self.convs = nn.Sequential(
            ...
            weight_norm(Conv2d(128, 256, (kernel_size, 1), (stride, 1), padding=(get_padding(5, 1), 0))),
            weight_norm(Conv2d(256, 512, (kernel_size, 1), (stride, 1), padding=(get_padding(5, 1), 0))),
            weight_norm(Conv2d(512, 1024, (kernel_size, 1), (stride, 1), padding=(get_padding(5, 1), 0))),
            weight_norm(Conv2d(1024, 1024, (kernel_size, 1), (stride, 1), padding=(get_padding(5, 1), 0))))
        self.conv_post = weight_norm(Conv2d(1024, 1, (3, 1), 1, padding=(1, 0)))

        init_weights_(self.convs)
        init_weights_([self.conv_post])
