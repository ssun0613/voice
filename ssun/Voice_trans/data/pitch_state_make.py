...
if __name__=='__main__':
    ...
    os.makedirs((os.path.join(path, "new/make_pitch(only_pitch_2)")), exist_ok=True)
    ...
    for index in range(0, len(dataset_path)):
        ...
        _f0, t = pw.dio(wav, fs)
        pitch = pw.stonemask(wav, _f0, t, fs)
        np.save(path + 'new/make_pitch(only_pitch_2)/{}_{}'.format(data.split('/')[-2], data.split('/')[-1][:-4]),pitch.astype(np.float32), allow_pickle=False)
        print("save {}_{}".format(data.split('/')[-2], data.split('/')[-1][:-4]))
    ...
    pitch_min, pitch_max = normalize(os.path.join(path, "new/make_pitch(only_pitch_2)"), pitch_mean, pitch_std)

    stats = {"pitch_min": float(pitch_min), "pitch_max": float(pitch_max), "pitch_mean": float(pitch_mean), "pitch_std": float(pitch_std)}
    np.save(path + 'new/pitch_state_2.npy',stats)
